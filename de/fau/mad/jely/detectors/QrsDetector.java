...
import de.fau.mad.jely.processors.PostProcessor;
import de.fau.mad.jely.processors.QrsDetectionPostProcessor;
...
public abstract class QrsDetector {
    ...
    /**
     * Constructs a QRS detector sensitive to the given sampling rate and
     * requiring signals from the given leads.
     *
     * @param samplingRate  the sampling rate in Hz for which this QRS detector should be
     *                      sensitive to.
     ...
     *                    EcgSignals required for this detector or -1 to use the last added sample.
    ...
    /**
     * This is called every time a new QRS complex has been found by the
     * detection algorithm.
     *
    ...
    protected void onQrsComplexFound(QrsComplex qrs) {
        ...
        if (mQrsList.size() > 0) {
             ...
             *
             * @param samplingRate
             *            the sampling rate in Hz for which this QRS detector should be
             *            sensitive to.
             * @param requiredLeads
             *            specifies leads from which signals are needed for this
             *            detector to work.
             */
	public QrsDetector( double samplingRate, LeadConfiguration requiredLeads){
                this(new Ecg(samplingRate, requiredLeads));
	...
	public QrsDetector(Ecg ecg) {
             ...
             *
             * @param sampleIndex
             *            the index of the most recently added signal sample in all
             *            EcgSignals required for this detector.
             * @return the detected QrsComplex, or null when no QrsComplex was detected
             *         after processing the sampled signal values.
             */
            public abstract QrsComplex next ( int sampleIndex);
             ...
             *
             * @param qrs
             *            the new QRS complex that was found.
             */
            protected void onQrsComplexFound (QrsComplex qrs){
                // TODO: maybe we should differentiate between found R-peaks and found
                // QrsComplexes. Some detectors only find
                ...
                if (mQrsList.size() > 0) {
                    ...
                    for (PostProcessor postProcessor : mPostProcessors) {
                        if (postProcessor instanceof QrsDetectionPostProcessor) {
                            int output = ((QrsDetectionPostProcessor) postProcessor).process(previousQrs);
                            // write some debug output?
/*					if (postProcessor instanceof RPeakRefinement && mDebugMode)
						System.out.println("Refinement distance: " + output);*/
                        }
                    }
                }
             ...
             *
             * @return the most recently found QRS complex, or <code>null</code> if no
             *         QRS complex has been detected so far
             */
            ...
            /**
             * Runs this QrsDetector on the entire signal array and returns a list of
             * all the found <code>QrsComplex</code>es. The signal is assumed to be
             * sampled at the proper sampling rate given at construction time of this
             * QrsDetector. The lead is assumed to be UNKNOWN. Use
             * findQrsComplexes(EcgSignal) instead, if you need the lead information.
             *
             * @param signal
             *            an array of sampled amplitude values for an ECG.
             * @return an ordered list of QrsComplexes found in the given signal by
             *         using this QrsDetector.
             */
